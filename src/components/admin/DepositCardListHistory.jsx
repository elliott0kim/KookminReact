...
const DepositCardListHistory = memo(()=>{
    const {setDepositCount} = useContext(TodoListCountHistoryContext);
    ...
    const fetchCardListData = async () => {
        try {
            const response = await axios.get(`/back/admin/stayDeposit/history`,{
            headers: {
    ...
    const handleBtnDeposit = (id)=>{
        const fetchData = async () => {
            try {
                if(!confirm(
                    `멘티님의 입금 확인을 취소 처리 하시겠습니까?`
                )){
                    console.log(id);
                    console.log("취소 작업이 중단되었습니다.");
                    return false;
                }
                const response = await axios.delete(`/back/admin/stayDeposit/history/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
        ...
        <>
            <div className="card-list-wrap" id="deposit">
                <div className="card-title-wrap">
                    ...
                    <p className="discript">멘티의 입금을 확인한 목록입니다.</p>
                </div>
                    ...
                    <div className="card deposit-card" key={card.reservationId}>
                        <div className="card-body">
                            ...
                            <div className="btns-wrap-hr">
                                ...
                                <button className="btn btn-secondary" onClick={() => handleBtnDeposit(card.newMentoringId)}>입금 확인 취소</button>
                            </div>
                        </div>
                    </div>
                ))}
...
export default DepositCardListHistory;
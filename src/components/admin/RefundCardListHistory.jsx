...
const RefundCardListHistory = memo(()=>{
    const {setRefundCount} = useContext(TodoListCountHistoryContext);
    ...
    const fetchCardListData = async () => {
        try {
            const response = await axios.get(`/back/admin/stayRefund/history`,{
            headers: {
    ...
    const handleBtnDeposit = (id)=>{
        const fetchData = async () => {
            try {
                if(!confirm(
                    `멘티님께 환불 완료 상태를 취소 처리 하시겠습니까?`
                )){
                    console.log("취소 작업이 중단되었습니다.");
                    return false;
                }
                const response = await axios.delete(`/back/admin/stayRefund/history/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                    ...
                    <div className="card deposit-card" key={card.refundId}>
                        <div className="card-body">
                            ...
                            <div className="btns-wrap-hr">
                                ...
                                <button className="btn btn-secondary" onClick={() => handleBtnDeposit(card.refundId)}>환불 완료 취소</button>
                            </div>
                        </div>
                    </div>
                ))}
...
export default RefundCardListHistory;